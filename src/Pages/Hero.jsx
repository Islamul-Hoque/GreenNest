...
const slides = [
  {
    ...
    image: "https://i.ibb.co.com/0pYSwQzs/Natural-Decor-for-Your-Home-1.png",
  },
  {
        ...
        <motion.div key={index} variants={slideVariants}  initial="initial"  animate="animate"  exit="exit"  transition={{ duration: 0.9, ease: [0.6, -0.05, 0.01, 0.9] }}  className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${slides[index].image})` }}  >
          ...
          <div className="relative h-full flex flex-col items-center justify-center text-white px-6 text-center">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}  transition={{ delay: 0.5 }} >
              <Link to="/plants" className="mt-8 btn shadow-none rounded-[0.55rem] outline-none border-none bg-green-700 text-white hover:bg-green-800"  > Explore Our Plants  </Link>
            </motion.div>
          </div>
        </motion.div>
      </AnimatePresence>
      <button  onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 p-3 rounded-full text-white transition z-10 hidden md:block"  ><ChevronLeft className="w-6 h-6 "/> </button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 p-3 rounded-full text-white transition z-10 hidden md:block" > <ChevronRight className="w-6 h-6"/> </button>
